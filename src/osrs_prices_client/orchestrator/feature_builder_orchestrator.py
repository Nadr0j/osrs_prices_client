...
class FeatureBuilderOrchestrator:
    """Orchestrates the scheduling and building of features
    using multiple FeatureBuilder instances."""

    @staticmethod
    def _schedule_feature_builders(
        data: pd.DataFrame, builders: list[FeatureBuilder]
    ) -> list[FeatureBuilder]:
        available_features = set(data.columns)
        pending_feature_builders = list(builders)
        scheduled_feature_builders = []
        ...
        while pending_feature_builders:
            ...
            if not progressed:  # We looped through all features and none were buildable
                ...
                raise ValueError(
                    "Unable to satisfy requirements for features: " + ", ".join(missing)
                )

        return scheduled_feature_builders

    @staticmethod
    def _build_features_for_identifier(
        identifier_data: pd.DataFrame, builders: list[FeatureBuilder]
    ) -> pd.DataFrame:
        scheduled_builders = FeatureBuilderOrchestrator._schedule_feature_builders(
            identifier_data, builders
        )
    ...
    def build_features(data: pd.DataFrame, builders: list[FeatureBuilder]) -> pd.DataFrame:
        identifiers = {col[0] for col in data.columns.to_list()}
        df = data.copy()
        parts: list[pd.DataFrame] = []

        for identifier in identifiers:
            identifier_data = df[identifier]
            identifier_data = FeatureBuilderOrchestrator._build_features_for_identifier(
                identifier_data, builders
            )
            identifier_data.columns = pd.MultiIndex.from_product(
                [[identifier], identifier_data.columns]
            )

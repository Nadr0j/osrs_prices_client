...
def test_forward_vwap_return_generates_shifted_returns():
    ...
    builder = features.ForwardVWAPReturn(horizons=[1, 2])
...
def test_forward_vwap_direction_flags_positive_returns():
    prices = pd.DataFrame({"volume_weighted_average_price": [100.0, 110.0, 121.0, 133.1]})
    builder = features.ForwardVWAPDirection(horizons=[1, 2])
...
def test_midpoint_price_builder_creates_expected_series():
    ...
    builder = features.MidpointPrice()
    result = builder.build(data)
...
def test_exponential_moving_average_builder_matches_pandas_ewm():
    ...
    builder = features.ExponentialMovingAverage(column="midpoint_price", span=2)
...
def test_simple_return_builder_matches_arithmetic_returns():
    ...
    builder = features.SimpleReturn(column="midpoint_price", horizons=[1, 2])
...
def test_log_return_builder_computes_expected_values():
    ...
    builder = features.LogReturn(column="midpoint_price", horizons=[1, 2])
...
def test_forward_log_return_builder_computes_expected_values():
    prices = pd.DataFrame({"midpoint_price": [10.0, 20.0, 40.0, 80.0]})
    builder = features.ForwardLogReturn(column="midpoint_price", horizons=[1, 3])
...
def test_rolling_volatility_builder_matches_manual_calculation():
    ...
    builder = features.RollingVolatility(column="midpoint_price", window=3, periods_per_year=365)
...
def test_rolling_mean_and_median_builders_compute_expected_values():
    ...
    mean_builder = features.RollingMean(column="midpoint_price", window=2, min_periods=None)
    median_builder = features.RollingMedian(column="midpoint_price", window=3, min_periods=2)
...
def test_rolling_price_channel_tracks_extrema():
    ...
    builder = features.RollingPriceChannel(column="midpoint_price", window=2)
...
def test_bollinger_bands_builder_matches_manual_calculation():
    ...
    builder = features.BollingerBands(column="midpoint_price", window=2, std_factor=2.0)
...
def test_average_true_range_builder_matches_hand_calculation():
    ...
    builder = features.AverageTrueRange(
...
def test_rolling_volume_builder_adds_total_and_mean():
    ...
    builder = features.RollingVolume(
...
def test_rate_of_change_builder_matches_formula():
    prices = pd.DataFrame({"midpoint_price": [10.0, 20.0, 40.0, 80.0]})
    builder = features.RateOfChange(column="midpoint_price", window=2)
...
def test_lag_feature_builder_creates_shifted_columns_with_optional_fill():
    ...
    builder = features.LagFeature(column="midpoint_price", lags=[1, 2], fill_method=None)
    ...
    builder_ffill = features.LagFeature(column="midpoint_price", lags=[1], fill_method="ffill")
...
def test_forward_vwap_return_metadata_reflects_requested_horizons():
    builder = features.ForwardVWAPReturn(horizons=[2, 4])
...
def test_forward_vwap_direction_metadata_reflects_requested_horizons():
    builder = features.ForwardVWAPDirection(horizons=[3])
...
def test_exponential_moving_average_metadata_and_validation():
    builder = features.ExponentialMovingAverage(column="midpoint_price", span=5)
    ...
    with pytest.raises(ValueError):
        features.ExponentialMovingAverage(column="midpoint_price", span=0)
...
def test_simple_return_metadata_and_validation():
    ...
    with pytest.raises(ValueError):
        features.SimpleReturn(column="midpoint_price", horizons=[0])


def test_log_return_metadata_and_validation():
    builder = features.LogReturn(column="midpoint_price", horizons=[2])
    assert builder.get_name() == "log_return_midpoint_price"
    assert builder.requires() == {"midpoint_price"}
    assert builder.provides() == {"log_return_midpoint_price_2"}

    with pytest.raises(ValueError):
        features.LogReturn(column="midpoint_price", horizons=[])
    with pytest.raises(ValueError):
        features.LogReturn(column="midpoint_price", horizons=[-1])


def test_forward_log_return_metadata_and_validation():
    builder = features.ForwardLogReturn(column="midpoint_price", horizons=[1])
    assert builder.get_name() == "forward_log_returns_midpoint_price"
    assert builder.requires() == {"midpoint_price"}
    assert builder.provides() == {"forward_log_returns_midpoint_price_1"}

    with pytest.raises(ValueError):
        features.ForwardLogReturn(column="midpoint_price", horizons=[])
    with pytest.raises(ValueError):
        features.ForwardLogReturn(column="midpoint_price", horizons=[0])


def test_forward_vwap_builders_validate_horizons():
    with pytest.raises(ValueError):
        features.ForwardVWAPReturn(horizons=[])
    with pytest.raises(ValueError):
        features.ForwardVWAPDirection(horizons=[])


def test_rolling_volatility_metadata_and_validation():
    builder = features.RollingVolatility(column="midpoint_price", window=5, periods_per_year=252)
    ...
    with pytest.raises(ValueError):
        features.RollingVolatility(column="midpoint_price", window=1, periods_per_year=252)
    with pytest.raises(ValueError):
        features.RollingVolatility(column="midpoint_price", window=5, periods_per_year=0)


def test_rolling_mean_and_median_validate_parameters():
    with pytest.raises(ValueError):
        features.RollingMean(column="midpoint_price", window=0, min_periods=1)
    with pytest.raises(ValueError):
        features.RollingMedian(column="midpoint_price", window=0, min_periods=1)


def test_rolling_price_channel_and_rate_of_change_metadata():
    channel = features.RollingPriceChannel(column="midpoint_price", window=3)
    ...
    with pytest.raises(ValueError):
        features.RollingPriceChannel(column="midpoint_price", window=0)

    roc = features.RateOfChange(column="midpoint_price", window=4)
    assert roc.get_name() == "roc_midpoint_price_4"
    assert roc.requires() == {"midpoint_price"}
    assert roc.provides() == {"roc_midpoint_price_4"}
    with pytest.raises(ValueError):
        features.RateOfChange(column="midpoint_price", window=0)


def test_bollinger_bands_average_true_range_and_volume_metadata():
    bands = features.BollingerBands(column="midpoint_price", window=5, std_factor=1.5)
    ...
    with pytest.raises(ValueError):
        features.BollingerBands(column="midpoint_price", window=1, std_factor=1.0)
    with pytest.raises(ValueError):
        features.BollingerBands(column="midpoint_price", window=5, std_factor=0)

    atr = features.AverageTrueRange(
        high_column="avgHighPrice",
        low_column="avgLowPrice",
        close_column="midpoint_price",
        window=14,
...
def test_lag_feature_metadata_and_validation():
    ...
    with pytest.raises(ValueError):
        features.LagFeature(column="midpoint_price", lags=[], fill_method=None)
    with pytest.raises(ValueError):
        features.LagFeature(column="midpoint_price", lags=[-1], fill_method=None)
    with pytest.raises(ValueError):
        features.LagFeature(column="midpoint_price", lags=[1], fill_method="invalid")
